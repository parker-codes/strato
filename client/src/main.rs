...
fn App() -> impl IntoView {
    let (game, set_game) = create_signal(bootstrap_game());
    let (mode, set_mode) = create_signal(TurnMode::Swap);
    let (message, set_message) = create_signal("Welcome to Strato!".to_string());

    let snapshot = create_memo(move |_| game.get().snapshot());
            ...
            GameState::Active | GameState::LastRound => {
                ...
                match g.end_player_turn(player_id, action) {
                    Ok(_) => set_message.set(format!(
                        ...
                        i32::from(card_in_hand.get_value().unwrap())
                    )),
                    Err(err) => set_message.set(err.to_string()),
                }
            }
    ...
    view! {
        main class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-slate-100 px-4 pb-12" {
            header class="max-w-6xl mx-auto flex items-center justify-between py-6" {
                div {
                    p class="text-sm uppercase tracking-wide text-slate-400" { "Skyjo-inspired" }
                    h1 class="text-4xl font-semibold" { "Strato" }
                }
...
fn GameMeta(snapshot: Memo<GameSnapshot>) -> impl IntoView {
    ...
    view! {
        section class="rounded-xl bg-slate-800/80 shadow-lg shadow-slate-950/40 border border-slate-700/50 p-4 flex flex-wrap items-center gap-4" {
            div class="flex-1" {
                p class="text-xs uppercase tracking-wide text-slate-400" { "State" }
                h2 class="text-xl font-semibold" { move || format!("{status:?}") }
            }
            div class="w-[1px] self-stretch bg-slate-700/50" {}
            div class="flex flex-col items-center justify-center" {
                p class="text-xs uppercase tracking-wide text-slate-400" { "Round" }
                p class="text-3xl font-bold" { move || round().to_string() }
            }
            div class="grid grid-cols-2 gap-3" {
                div class="p-3 rounded-lg bg-slate-900/60 border border-slate-700/50" {
                    p class="text-xs uppercase tracking-wide text-slate-400" { "Deck" }
                    p class="text-lg font-semibold" { move || format!("{} cards", deck_size()) }
                }
                div class="p-3 rounded-lg bg-slate-900/60 border border-slate-700/50" {
                    p class="text-xs uppercase tracking-wide text-slate-400" { "Discard" }
                    p class="text-lg font-semibold" { move || format!("{} cards", discard_size()) }
                    div class="mt-2" {
            ...
            <For
                ...
                children=move |player: PlayerView| {
                    let idx = snapshot()
                        ...
                        .unwrap_or(0);
                    view! {
                        PlayerPanel { player, is_current: move || snapshot().current_player_idx == Some(idx), on_cell_click: on_cell_click.clone(), player_idx: idx }
                    }
                }
            />
        }
    ...
    view! {
        article class="rounded-xl overflow-hidden border border-slate-700/60 bg-slate-800/60 shadow-lg shadow-slate-950/30" {
            div class=move || format!("flex items-center justify-between px-4 py-2 text-sm font-semibold {}", header_class()) {
                span { name.clone() }
                span class="text-xs" { move || format!("{} flipped", flipped) }
            }
            div class="p-4 space-y-3" {
                div class="flex items-center gap-2 text-sm text-slate-300" {
                    span { "Score:" }
                    span class="text-lg font-semibold text-white" { score }
    ...
    view! {
        aside class="h-full space-y-4" {
            div class="rounded-xl bg-slate-800/80 border border-slate-700/70 p-4 shadow-md shadow-slate-950/30" {
                h3 class="text-lg font-semibold mb-2" { "Turn actions" }
                div class="space-y-2" {
                ...
                div class="pt-3 border-t border-slate-700/60 mt-3 space-y-2" {
                    p class="text-xs uppercase tracking-wide text-slate-400" { "Placement mode" }
            ...
            div class="rounded-xl bg-slate-900/80 border border-slate-800 p-4 text-sm shadow-inner shadow-slate-950/30 min-h-[120px]" {
                p class="text-xs uppercase tracking-wide text-slate-400" { "Game log" }
                p class="mt-2 text-slate-200" { message }
            }
        }
    }
}
...
fn CardView(slot: Option<CellView>) -> impl IntoView {
    let content = move || match slot {
        None => view! { div class="h-full w-full bg-slate-900/50" {} },
        Some(CellView::Empty) => view! { div class="h-full w-full bg-slate-900/50" {} },
        Some(CellView::Hidden) => view! { CardBack {} }.into_view(),
        Some(CellView::Revealed(value)) => view! { CardFace { value } }.into_view(),
    };

    view! {
        div class="relative h-full w-full" { (content)() }
    }
}
...
fn CardBack() -> impl IntoView {
    view! {
        div class="h-full bg-slate-900 relative" style=format!("background-image: {}", HEX_PATTERN) {
            div class="absolute inset-[5%] border-4 border-white opacity-40" {}
            span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-5xl font-bold text-white" { "S" }
        }
    }
}
